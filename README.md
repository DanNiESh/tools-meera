...
To test if the ope is installed, run ```ope```, if it is installed, a list of ope commands should print on your terminal

### How to create a new project:
1. Create a new github repository from the web UI 
2. In the terminal, run ```ope new_project [repo-url]```
   * Note: the repo-url is an optional argument, if you would like to add it later on, use the `ope repo_add` command later
...
1. Enter back into the terminal and cd into the `books` folder and the run: ```ope new_book <your-book-name>```
2. Your book is now made, cd into the `content` folder, three files must be modified:
   - your-text-book-name_config.yml file: edit the title, name, url and copyright year
   - your-text-book-name_intro.md file: this file will be your books introduction, edit it to populate your landing page
   - your-text-book-name_toc.yml: this file will contain your intended structure for your textbook
     - To add chapters or sections to this book, add the text under this line, under parts, where each file or section is the path to the chapter content
     ```
...
3. To have content where that chapter will be, create a folder in the content folder, following the path you wrote in the previous step
...
4. Continue this process until your textbook structure is layed out and your folders are populated with the corresponding .ipynb's
5. To see what your textbook looks like, go into the books folder and run ```ope build``` and copy and paste the provided link in your browser
...
If you have any other questions or need additional help in the process: checkout this [video](https://www.youtube.com/watch?v=-_0HD_oAK5o&list=LL)
## Usage
